...
{
    return [self addHCustomView:customView onRect:rect isBorder:NO];
}

- (NSInteger)addHCustomView:(UIView *)customView onRect:(CGRect)rect isBorder:(BOOL)isBorder
{
...
- (void)addCustomViews
{
    [self.holes enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[JMCustomRectHole class]]) {
            ...
            if (hole.isBorder) {
                CGRect holeRectIntersection = CGRectIntersection(hole.holeRect, self.frame);
                UIRectFill( holeRectIntersection );
            }
        }
    }];
