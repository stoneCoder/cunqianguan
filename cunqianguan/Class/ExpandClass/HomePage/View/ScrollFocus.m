...
//
static CGFloat SWITCH_FOCUS_IMAGE_INTERVAL = 4.0; //switch interval time
...
-(void)buidView{
    ...
    [_pageControl setPageIndicatorImage:[UIImage imageNamed:@"qiehuan_home"]];
    [_pageControl setCurrentPageIndicatorImage:[UIImage imageNamed:@"qiehuan_hover"]];
...
-(void)setTitleArray:(NSArray *)titleArray{
    ...
    float pageControlWidth = (pageCount) * 18.0f + 5*(pageCount + 1);
    float pagecontrolHeight = 20.0f;
    
    _pageControl.frame = CGRectMake(0,0, pageControlWidth + 20, pagecontrolHeight);
    _pageControl.numberOfPages = (pageCount);
    _pageControl.currentPage = 0;
    _pageControl.center = CGPointMake(_noteView.frame.size.width/2, _noteView.frame.size.height/2);
